...
words_train, tags_train = get_data('../data/pos/train_word.pkl',
                           '../data/pos/train_tag.pkl')
words_val, tags_val = get_data('../data/pos/dev_word.pkl',
                           '../data/pos/dev_tag.pkl')
vocab, reversed_vocab = get_char_vocab('../data/pos/char-vocab.txt')
...
x_val = transform(words_val, vocab)
...
def cnn_layer(x):
    filter_weights = {
        'fw' + str(i + 1): tf.Variable(tf.random_uniform(
            [FILTER_SIZE[i], EMBEDDING_SIZE, 1, NUM_FILTER[i]], -0.05, 0.05),
            name='fw' + str(i + 1)) for i in range(len(FILTER_SIZE))
    }

    filter_bias = {
        'fb' + str(i + 1): tf.Variable(
            tf.random_uniform([NUM_FILTER[i]], -0.05, 0.05), name='fb' + str(i + 1))
...
def highway_layer(x):
    t1_weight = tf.Variable(tf.random_uniform([TOTAL_FILTERS, TOTAL_FILTERS],
                        -0.05, 0.05),
                        name='t1_weights')
    t1_bias = tf.get_variable(initializer=tf.random_uniform([TOTAL_FILTERS], -0.05, 0.05),
                             name='t1_bias')
    transform_gate_1 = tf.nn.sigmoid(tf.matmul(x, t1_weight) + t1_bias)
    h1_weight = tf.Variable(tf.random_uniform([TOTAL_FILTERS, TOTAL_FILTERS],
                        -0.05, 0.05), name='h1_weights')
    h1_bias = tf.get_variable(initializer=tf.random_uniform([TOTAL_FILTERS], -0.05, 0.05),
                             name='h1_bias')
    highway_output = transform_gate_1 * tf.nn.relu(tf.matmul(x, h1_weight) + h1_bias) \
                        + (1 - transform_gate_1) * x
    '''
    t2_weight = tf.get_variable(shape=[TOTAL_FILTERS, TOTAL_FILTERS],
                        initializer=tf.contrib.layers.xavier_initializer(),
                        name='t2_weights')
    t2_bias = tf.get_variable(initializer=tf.constant(0.1, shape=[TOTAL_FILTERS]),
                             name='t2_bias')
    transform_gate_2 = tf.nn.sigmoid(tf.matmul(highway_output, t2_weight) + t2_bias)
    h2_weight = tf.get_variable(shape=[TOTAL_FILTERS, TOTAL_FILTERS],
                        initializer=tf.contrib.layers.xavier_initializer(),
                        name='h2_weights')
    h2_bias = tf.get_variable(initializer=tf.constant(0.1, shape=[TOTAL_FILTERS]),
                             name='h2_bias')
    highway_output = transform_gate_2 * tf.tanh(tf.matmul(highway_output, h2_weight) + h2_bias) \
                        + (1 - transform_gate_2) * highway_output
    '''
...
def fc_layer(x):
    fc_weight = tf.Variable(tf.random_uniform([LSTM_HIDDEN_UNIT, NUM_CLASSES],
                                -0.05, 0.05), name='fc_weight')
...
val_size = len(words_val)
num_batch_val = int((val_size - 1) / NUM_WORD_INPUT) + 1
best_acc = 0
for epoch in range(0, NUM_EPOCH + 1):
    ...
    if epoch % 5 == 0:
        ...
        acc = []
        for k in range(num_batch_val):
            start_idx = k * (NUM_WORD_INPUT)
            end_idx = k * (NUM_WORD_INPUT) + NUM_WORD_INPUT
            if end_idx >= val_size:
                end_idx = val_size
                start_idx = end_idx - NUM_WORD_INPUT
            cp = sess.run([correct_predictions],
                        feed_dict={input_sentences:x_val[start_idx:end_idx],
                                   output_tags:tags_val[start_idx:end_idx],
                                   keep_pr:1.0})
            acc.append(float(np.sum(cp)) / len(cp[0]))
        val_accuracy = float(sum(acc)) / len(acc)
