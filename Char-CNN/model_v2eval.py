...
words_test, tags_test = get_data('../data/x_test.pkl',
                           '../data/y_test.pkl')
...
def lstm_layer(x):
    ...
    #    for _ in range(LSTM_NUM_LAYER)], state_is_tuple=True)
    x = tf.unstack(tf.reshape(x, [BATCH_SIZE, WORD_PER_BATCH, TOTAL_FILTERS]))
    output, _ = tf.contrib.rnn.static_rnn(fw_cell, x, dtype=tf.float32)
    output = tf.reshape(tf.stack(output), [-1, LSTM_HIDDEN_UNIT])
    return output
