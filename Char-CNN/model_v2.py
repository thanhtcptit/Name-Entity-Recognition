...
def get_data(words_file, tags_file):
    words = pkl.load(open(words_file, 'rb'))
    tags = pkl.load(open(tags_file, 'rb'))
    return words, tags
...
words_train, tags_train = get_data('../data/x_train.pkl',
                           '../data/y_train.pkl')
words_val, tags_val = get_data('../data/x_valid.pkl',
                           '../data/y_valid.pkl')
...
output_tags = tf.placeholder(tf.int32,
                    [None, NUM_CLASSES],
                    'output_tags')
sentences_length = tf.placeholder(tf.int32, [None])
...
for epoch in range(1, NUM_EPOCH + 1):
    ...
    for j in range(NUM_BATCH):
        ...
        for k in range(BATCH_SIZE):
            count = 0
            for x in words_batch[k * WORD_PER_BATCH: k * WORD_PER_BATCH + WORD_PER_BATCH]:
                if sum(x) != 0:
                    count += 1
            sentences_len.append(count)
        ...
        _, current_step, batch_loss, batch_acc = sess.run(
                        ...
                        feed_dict={input_sentences:words_batch,
                                   ...
                                   keep_pr:KEEP_PROB,
                                   sentences_length:sentences_len})
